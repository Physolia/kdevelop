...
namespace Config {
namespace Strings {
QByteArray sourceFormatter() { return QByteArrayLiteral("SourceFormatter"); }
...
void populateStyleFromConfig(SourceFormatterStyle& style, const KConfigGroup& styleConfig)
{
    ...
    style.setMimeTypes(styleConfig.readEntry<QStringList>(SourceFormatterController::styleMimeTypesKey(), QStringList{}));
...
struct FormatterData
{
    ...
    bool isValid() const { return formatter; }
...
FormatterData readFormatterData(const KConfigGroup& sourceFormatterConfig,
...
SourceFormatterController::FileFormatter::FileFormatter(QUrl&& url,
                                                        ...
                                                        const ISourceFormatter* formatter,
...
{
    Q_ASSERT_X(!m_sourceFormatterConfig.isValid() && !m_formatter && m_style.name().isEmpty(),
               Q_FUNC_INFO, "This reinitialization must be a mistake.");
...
QString SourceFormatterController::FileFormatter::format(const QString& text,
...
QString SourceFormatterController::FileFormatter::addModeline(QString input) const
{
    ...
    QString modeline(QStringLiteral("// kate: ")
                     + QLatin1String("indent-mode ") + indentationMode(m_mimeType) + QLatin1String("; "));
...
void SourceFormatterController::beautifySource()
{
    ...
    if (has_selection) {
        ...
        QString output = ff.format(view->selectionText(),
...
void SourceFormatterController::FileFormatter::adaptEditorIndentationMode(KTextEditor::Document* doc, bool ignoreModeline) const
{
    Q_ASSERT(m_formatter);
    if (!doc || !m_sourceFormatterConfig.readEntry(SourceFormatterController::kateOverrideIndentationConfigKey(), false))
        return;
